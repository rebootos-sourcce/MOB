...
11. **`impRender()` is O(pills × engine).** `impPill` builds a `title` attribute by calling
    `limitOf(n)`, and `limitOf` calls `compute()` — a full 108-address, four-tier chain
    rebuild — once per pill. `painOf` walks all 108 again per pill. At ~50 held addresses
...
2. **Size is the real gap.** Mono eyebrows are set at 7px, 7.5px and 8px. Body rows are 9px
   to 10.5px. The direction sets reading text at 15–16px and small text at 14px. Every text
