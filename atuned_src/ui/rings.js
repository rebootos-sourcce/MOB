...
const FVIEWS=[
 ...
 {k:'dial',nm:'Dial',
  ...
  tip:'The ring at full size with every layer showing, and the heaviest few named in the corners.'}];
var FVIEW='wheel';
...
const FLAYS=[
 ...
 {k:'gates',nm:'Gates',d:frCirc(12,12,3.1)+'M12 8.9V3.6M9.6 6L12 3.6 14.4 6M12 15.1v5.3',
  tip:'Three higher gates above the core and three lower below it.'},
 {sep:true},
...
/* THE CORE. The product's own ring grammar at hero size: an arc for the
   share, the number inside it in the tier's colour, and its scale under it,
   because every number says what it is out of. The range the coherence scale
   draws as its pill rides outside the arc, drawn and never stated, and it is
   the compass's own arithmetic, cqRange, so the two cannot disagree.

   NOTHING IS PRINTED OFF THE DEFAULTS. On an unread field the ring is empty,
   the range is not drawn and the figure is a dash, which is what the key pill
   directly above says in the same state. */
function frCore(M,r,cx,cy,R,o){
 var w=o.w,un=!!r.unread,f=clamp(r.CQ/100,0,1);
 ...
 if(!un){var rg=cqRange(r.CQ),rr=R+w/2+o.halo/2;
  s+='<path d="'+frArcD(cx,cy,rr,-Math.PI/2+TAU*rg.lo/100,-Math.PI/2+TAU*rg.hi/100)+'" fill="none" stroke="'
   +rgba(col,.55)+'" stroke-width="3" stroke-linecap="round"/>';}
 var fs=o.fs,cap=fs*.727,sub=12,gap=Math.max(8,fs*.16),top=cy-(cap+(un?0:gap+sub*.72))/2;
 s+='<text x="'+cx.toFixed(1)+'" y="'+(top+cap).toFixed(1)+'" text-anchor="middle" fill="'+frRgb(col)
  +'" font-size="'+fs.toFixed(1)+'" font-weight="300" letter-spacing="-0.02em">'+(un?'\u2013':Math.round(r.CQ))+'</text>';
 if(!un)s+='<text x="'+cx.toFixed(1)+'" y="'+(top+cap+gap+sub*.72).toFixed(1)+'" text-anchor="middle" fill="'
  +frRgb(M.dim)+'" font-size="'+sub+'" font-weight="500">of 100</text>';
 M.L.core.push(s+'</g>');}
/* ONE GATE, verpArrows in ui/wheel.js: the ring, the glyph, and the pill that
   holds a dash until a story has run through the gate. The pill's figure is
   set at 11 pixels here. The wheel draws it at 8.5, under the product's own
   floor, where the design gate cannot see it because it is canvas, and BO2
   records why the wheel has not moved: at 11 its pills sit under the next
   gate's disc. Here the gates are spaced by their own pills instead. */
function frGate(M,x,y,v,R,G){
 var ev=G.ev,up=v.side==='higher',c=M.seat(up?'Heart':'Root'),f=(ev?v.pct:0)/100;
 var s='<g data-h="'+M.hid({k:'gate',v:v})+'">'
  +'<circle cx="'+x.toFixed(1)+'" cy="'+y.toFixed(1)+'" r="'+R.toFixed(1)+'" fill="'+frRgb(M.ground)+'"/>'
  +'<circle cx="'+x.toFixed(1)+'" cy="'+y.toFixed(1)+'" r="'+R.toFixed(1)+'" fill="none" stroke="'+rgba(c,ev?.22:.46)+'" stroke-width="2.2"/>';
 if(f>0)s+='<path d="'+frArcD(x,y,R,-Math.PI/2,-Math.PI/2+TAU*f)+'" fill="none" stroke="'+rgba(c,.95)+'" stroke-width="2.2" stroke-linecap="round"/>';
 s+=frGlyph(GATEGLYPH[v.k],x,y,R*1.15,rgba(M.ink,ev?.95:.86),1.6);
 var lab=G.lab(v),pw=G.pw(v),ph=15,px=x+R-4,py=y+R-7;
 s+='<rect x="'+px.toFixed(1)+'" y="'+py.toFixed(1)+'" width="'+pw+'" height="'+ph+'" rx="'+(ph/2)+'" fill="'
  ...
  +(ev?'rgb(16,16,16)':rgba(M.ink,.8))+'">'+lab+'</text></g>';
 M.L.gates.push(s);}
...
function frFrames(M,r){
 ...
 var R=76*sc;
 frCore(M,r,cx,cy,R,{w:6*Math.max(sc,.6),fs:Math.max(20,64*sc),halo:10*Math.max(sc,.6)});
 /* SPACED BY THEIR PILLS. The mockup's fifty pixels hold a 24 wide pill at
    full size. Scaled down they did not: measured over all fifteen reference
    cases at 390, a pill sat under the next gate's disc sixty times. A pill
    hangs from its gate's lower right, so the next gate stands at least two
    radii and a pill away, which changes nothing where fifty already cleared. */
 (function(){var G=frGates(),Rg=12*Math.max(sc,.75),dy=R+33*sc,dx=Math.max(50*sc,2*Rg+G.pwMax-2);
  G.hi.forEach(function(v,k){frGate(M,cx+(k-(G.hi.length-1)/2)*dx,cy-dy,v,Rg,G);});
  G.lo.forEach(function(v,k){frGate(M,cx+(k-(G.lo.length-1)/2)*dx,cy+dy,v,Rg,G);});})();}
...
function frDial(M,r){
 ...
 frCore(M,r,cx,cy,Rr.core,{w:5*Math.max(sc,.6),fs:Math.max(20,56*sc),halo:10*Math.max(sc,.6)});
 /* SPACED BY THEIR PILLS, the frames' rule on an arc. A pill hangs to its
    gate's right, so the next gate round has to stand two radii and a pill
    along: the step opens past the mockup's 0.66 only when the arc is too
    small to hold that, which at 1600 it never is and at 390 it always is. */
 (function(){var G=frGates(),Rg=11.5*Math.max(sc,.75);
  var step=Math.max(.66,Math.asin(Math.min(1,(2*Rg+G.pwMax-2)/Rr.gate)));
  var put=function(list,up){list.forEach(function(v,i){var a=(up?-Math.PI/2:Math.PI/2)+(i-(list.length-1)/2)*step;
   frGate(M,cx+Math.cos(a)*Rr.gate,cy+Math.sin(a)*Rr.gate,v,Rg,G);});};
  put(G.hi,true);put(G.lo,false);})();

 frCallouts(M,r,{cx:cx,cy:cy,R0:R0,C:C,tip:tip,anchorOf:anchorOf,archAt:archAt,lawAt:lawAt,at:at,hitOf:hitOf});}
...
/* ---- the callouts. what is worth a name goes to the nearest corner ----
   ...
   so no two leaders cross, and each belongs to its layer and goes with it.
   ...
   A NAME THAT DOES NOT FIT IS NOT DRAWN. The mockup's corners were set on a
   stage 892 across. The wheel's cell is narrower, and a dial that fills it
   leaves less corner, so each label is measured and tested against the rim
   before it is placed, and a corner that runs out of room stops. What is not
   named here is still in the readout and in the rail. */
function frCallouts(M,r,P){
 ...
 var groups={tl:[],tr:[],bl:[],br:[]};
 items.forEach(function(it){groups[(it.y>=cy?'b':'t')+(it.x>=cx?'r':'l')].push(it);});
 var rim=P.C(P.R0+10),clear=P.R0+6;
 Object.keys(groups).forEach(function(c){var list=groups[c],right=c[1]==='r',below=c[0]==='b';
  ...
  list.sort(function(a,b){return Math.abs(Math.sin(b.th))-Math.abs(Math.sin(a.th));});
  for(var i=0;i<list.length;i++){var it=list[i];
   var lx=right?w.x1-6:w.x0+6,ly=below?w.y1-10-i*38:w.y0+22+i*38;
   var tw=Math.max(frTextW(it.nm,'600 12.5px Inter, system-ui, sans-serif'),frTextW(it.sub,'500 11px Inter, system-ui, sans-serif'));
   var bx0=right?lx-tw:lx,bx1=right?lx:lx+tw,by0=ly-21,by1=ly+9;
   var nx=Math.max(bx0,Math.min(cx,bx1)),ny=Math.max(by0,Math.min(cy,by1));
   if(Math.hypot(nx-cx,ny-cy)<=clear||bx0<w.x0||bx1>w.x1||(below?by0<cy:by1>cy))break;
   var e=rim.at(it.th),ey=ly-4,g2=M.T[it.k]||M.T.seats;
   ...
   g2.push('<g data-h="'+it.h+'" data-call="'+it.k+'"><circle cx="'+it.x.toFixed(1)+'" cy="'+it.y.toFixed(1)+'" r="2.4" fill="'+frRgb(it.col)+'"/>'
    +'<path d="M'+it.x.toFixed(1)+' '+it.y.toFixed(1)+'L'+e.x.toFixed(1)+' '+ey.toFixed(1)+'L'+lx.toFixed(1)+' '+ey.toFixed(1)
    +'" fill="none" stroke="'+rgba(it.col,.45)+'" stroke-width="1"/>'
    +frLabel(M,it.nm,lx,ly-9,'text-anchor="'+(right?'end':'start')+'" font-size="12.5" font-weight="600" fill="'+frRgb(mixc(it.col,M.ink,.18))+'"')
    +frLabel(M,it.sub,lx,ly+6,'text-anchor="'+(right?'end':'start')+'" font-size="11" font-weight="500" fill="'+frRgb(M.dim)+'"')
    +'</g>');}});}
