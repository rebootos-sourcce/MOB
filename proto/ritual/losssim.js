...
const CHAIN=require(path.resolve(__dirname,'../game/chain.js'));
const STEM=require(path.resolve(__dirname,'../game/stem.js'));
const FRAMES=require(path.resolve(__dirname,'../game/frames.js'));
...
/* THE OWNER'S RULING, MODELLED AS HE STATED IT. "If I fail an accountability
   I lose points, if I succeed I gain." The existing REFUSED arm is the Patel
   uplift with no downside at all, so it is a ceiling and not his mechanic. His
   mechanic has a second half: a deduction lands on a miss. sting is that
   deduction as extra one day churn, discounted by what the person has banked
   the same way the run break shock is, and it stacks on the run break rather
   than replacing it, because a person who misses and loses points and watches
  ...
  for(let day=1;day<=90;day++){
   ...
   if(SPEC){
    ...
    if(!did&&streak>0&&grace===0){
     ...
     brokeToday=true;}
    /* THE DEDUCTION. It lands on EVERY miss, which is the difference between
       his mechanic and the run break shock above: the run break needs a live
