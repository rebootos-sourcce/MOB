#!/usr/bin/env node
/* Shoots the dashboard and measures it in the same run, because a screenshot
   taken in a different session from the measurement is two claims about two
   builds. Asserts, and exits non zero on any of them:
...
const URL='file://'+path.join(__dirname,'dash.html');
...
(async()=>{
 ...
 for(const w of [[1600,1000],[390,844]]){
  ...
  for(const who of WHO){
   ...
   await pg.screenshot({path:path.join(__dirname,'dash-'+tag+'-'+w[0]+'.png'),fullPage:true});
   /* and one worked, so the moved bar is on the record too */
   if(who!=='__empty'){
    for(let i=0;i<12;i++){await pg.click('#run');}
    await pg.waitForTimeout(400);
    await pg.screenshot({path:path.join(__dirname,'dash-'+tag+'-worked-'+w[0]+'.png'),fullPage:true});
    await pg.click('#reset');
   }
  }
  /* the measurements, on the loaded case */
  ...
  if(outside.length||errs.length||m.small.length||m.reqs)bad++;
  await ctx.close();
 }
 await b.close();
