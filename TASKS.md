...
- [x] **QC1. His words: "I don't know what A2 kept is. I don't know what does
      ...
      wearing one. Answered by QC2 and QC3 below, and the three he named are
      the three that were rewritten first.
- [x] **QC2. Every question carries a snapshot. Built, and the count is off
      the run.** His word. Four parts on every one: what the thing is, for
      somebody who has not read the finding; what is at stake; what happens
      either way, as two branches rather than an implication; and the file to
      open, because half of these had a drawing in `proto/` nobody had pointed
      him at. 115 paths are named across them and every one was checked to
      exist. The cross references between questions are by identifier and
      never by line number, because the identifiers hold and the lines moved
      by 544 the moment the snapshots landed, which is this repository's own
      recurring defect caught before it shipped.
- [x] **QC3. And the generator gates it. Built, and it bites.** `[?]` with no
      `>` block, or a `>` block missing any of the four names, fails the gate
      by identifier and nothing is written. Proved by planting two lines, one
      with no snapshot and one three parts of four: both were named, both by
      identifier and `TASKS.md` line, and neither output file was touched.
      A snapshot is the indented run of lines beginning with `>`, which is a
      markdown block quote in any viewer and the one opening a finding's own
      continuation cannot have, so the tool tells them apart by shape.
